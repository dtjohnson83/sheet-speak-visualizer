...
// Secure version that sanitizes CSS content
      ...
      .map(([theme, prefix]) => {
        const themeStyles = colorConfig
          .map(([key, itemConfig]) => {
            ...
            return `  --color-${sanitizeCSSSelector(key)}: ${sanitizeCSSValue(color)};`
          })
          .filter(Boolean)
          .join('\n')
        ...
        return `${prefix} [data-chart="${sanitizeCSSSelector(id)}"] {\n${themeStyles}\n}`
      })
      .filter(Boolean)
      .join('\n')

    // Set content directly without using dangerouslySetInnerHTML
...
// Security utility functions for CSS
