...
export const TileSankeyChartRenderer = ({ 
  ...
  console.log('TileSankeyChartRenderer - Input data:', {
    sankeyTargetColumn,
    dataType: typeof data,
    isArray: Array.isArray(data),
    hasNodes: data && typeof data === 'object' && 'nodes' in data,
    hasLinks: data && typeof data === 'object' && 'links' in data,
    data: Array.isArray(data) ? data.slice(0, 2) : data
  });

  // Check if sankeyTargetColumn is configured
  ...
  // Handle structured Sankey data (from prepareChartData)
  if (data && typeof data === 'object' && !Array.isArray(data) && 'nodes' in data && 'links' in data) {
  ...
  // Handle array data (fallback - should process this into Sankey format)
  if (Array.isArray(data) && data.length > 0) {
    // Process sankey data from raw array
  ...
  return (
    <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-400 text-sm">
      No data available for Sankey chart
    </div>
  );
};
