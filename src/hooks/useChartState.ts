...
export const useChartState = () => {
  const [chartType, setChartType] = useState<'bar' | 'line' | 'area' | 'pie' | 'scatter' | 'heatmap' | 'stacked-bar' | 'treemap' | 'treemap3d' | 'histogram' | 'kpi' | 'bar3d' | 'scatter3d' | 'surface3d' | 'timeseries3d' | 'network' | 'network3d' | 'entity-relationship' | 'map2d' | 'map3d'>('bar');
  ...
  const dataLabelSupportedCharts = ['bar', 'line', 'area', 'pie', 'stacked-bar', 'histogram', 'treemap', 'treemap3d', 'bar3d', 'scatter3d', 'timeseries3d', 'network', 'network3d', 'entity-relationship', 'map2d', 'map3d'];
